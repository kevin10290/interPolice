...
//devolver la ruta de la imagen desde la API
people.get("/people/sendimage/:photo", (req, res) => {
  //parametro
  let photo = req.params.photo;
  //ruta
    let ruta = "./uploads/people/"+photo;
    fs.access(ruta,(error)=>{
        if(!error){
            ...
            res.status(404).send({
                status: "error",
                message: "no exite la imagen solicitada",
              });
        }
    })
  //acceso y retorna de la imagen usando su ruta
